...
fn update_bitmap(bitmap: &Vec<u8>, new_bitmap: &mut Vec<u8>) {
    ...
    for i in (0..bitmap.len()).step_by(old_bytes_per_row / 2) {
        for j in 0..new_bytes_per_row / 2 {
            if j < old_bytes_per_row / 2 {
    ...
    // removing 500_000 to 625_000 (empty values, this can change)
